...
import 'package:calendrierbiblic/widgets/language_view.dart';
...
class _FirstStepPageState extends State<FirstStepPage> {
  ...
  Widget build(BuildContext context) {
    ...
    _btText = isEnd ? S.of(context).start : S.of(context).ignore;
          ...
          children: [
            Positioned(
                ...
                child: SingleChildScrollView(
                    child: SizedBox(height: screen.height > 390 ? screen.height - 30 : 360, child: showPageView()))),
    ...
    if (that.viewState == 1)
      return BibleTranslatesPage(
          afterDownload: (_) async {
    ...
    return DefaultTabController(
        ...
        child: Builder(builder: (BuildContext context) {
          ...
          defaultCtrl.addListener(() {
            if (defaultCtrl.index == displays.length - 1 && !isEnd) return setState(() => isEnd = true);
            if (defaultCtrl.index != displays.length - 1 && isEnd) return setState(() => isEnd = false);
          });
  ...
  void goToHome() => Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => HomePage()));
}
